...
namespace DAO
{
    public class AccReceiveWriteoffDAO:BaseClass
    {
        private const string tableName = "ACC_RECEIVE_WRITEOFF";
        private List<string> PK;

        public AccReceiveWriteoffDAO()
        {
            PK = CommDAO.getPK(comm, tableName);
        }

        #region Batch call
        public bool isExist(AccReceiveWriteoff_item data)
        {
            return CommDAO.isExist(comm, data, tableName, PK);
        }

        public bool AUD(AccReceiveWriteoff_item data, string employeeNo, string name, CommDAO dao = null)
        {
            switch (data.State)
            {
        ...
        private bool _AddBatch(AccReceiveWriteoff_item data, CommDAO dao = null)
        {
            return CommDAO._add(comm, data, tableName, PK, dao);
        }
        private bool _UpdateBatch(AccReceiveWriteoff_item data, CommDAO dao = null)
        {
            return CommDAO._update(comm, data, tableName, PK, null, dao);
        }
        private bool _DeleteBatch(AccReceiveWriteoff_item data, CommDAO dao = null)
        {
            return CommDAO._delete(comm, data, tableName, PK, null, dao);
        }
        public DataTable _QueryBatch(string RECW_COMPID, string RECW_NO)
        {
            if (string.IsNullOrEmpty(RECW_COMPID))
            {
                Log.Info("_QueryBatch: 無公司代號");
                return null;
            }
            if (string.IsNullOrEmpty(RECW_NO))
            {
...
WHERE 1=1 
AND RECW_COMPID='{RECW_COMPID}' ";
            sql += CommDAO.sql_ep(RECW_NO, "RECW_NO");

            return comm.DB.RunSQL(sql);
        }
        #endregion

        public bool DeleteAll(string RECW_COMPID, string RECW_NO, CommDAO dao)
        {
            string sql = $@"DELETE {tableName} WHERE RECW_COMPID ='{RECW_COMPID}' AND RECW_NO ='{RECW_NO }'";
